      ...
      case EVENT_CATEGORY_INFO_STATUS:
         {
            ...
            UtlString* pString = allocInfoStatusEvent(pStateInfo->pCookie, 
               pStateInfo->status, 
      ...
      case EVENT_CATEGORY_KEEPALIVE:
         {
            ...
            UtlString* pString = allocKeepaliveEvent(pKeepaliveInfo->event,
               ...
               pKeepaliveInfo->type,
               pKeepaliveInfo->szRemoteAddress,
               pKeepaliveInfo->remotePort,
               pKeepaliveInfo->keepAliveSecs);
...
UtlString* EventValidator::allocInfoStatusEvent(void* pCookie, int status, int responseCode, const char* szResponseText) 
{
   char szBuffer[1024];    

   sprintf(szBuffer, "<INFO STATUS> pCookie=%d: status=%d, responseCode=%d, responseText=%s",
      pCookie, 
      status, 
      responseCode, 
...
UtlString* EventValidator::allocConfigEvent(SIPX_CONFIG_EVENT hEvent)
{
   ...
   sprintf(szBuffer, "<CONFIG> event=%s",
      sipxConfigEventToString(hEvent));
                                              ...
                                              SIPX_SECURITY_CAUSE cause)
{
   ...
   sprintf(szBuffer, "<SECURITY> event=%s, cause=%s",
      sipxSecurityEventToString(event),
      sipxSecurityCauseToString(cause));
                                               ...
                                               int keepAliveSecs)
{
   ...
   case SIPX_KEEPALIVE_CRLF:
      strcpy(szType, "SIPX_KEEPALIVE_CRLF");
      break;
   ...
   sprintf(szBuffer, "<KEEPALIVE> event=%s, cause=%s type=%s szRemoteAddress=%s remotePort=%d keepAliveSecs=%d",
      sipxKeepaliveEventToString(event),
      sipxKeepaliveCauseToString(cause),
      szType, szRemoteAddress, remotePort, keepAliveSecs);
...
{ 
   ...
   sprintf(szBuffer1, "<SUBSTATUS> state=%s, cause=%s", 
      sipxSubStatusStateToString(state), 
      sipxSubStatusCauseToString(cause)); 

   return new UtlString(szBuffer1); 
} 
