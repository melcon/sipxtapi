...
#define STRESS_FACTOR   5       /**< How many times to repeat each test case */
#define TEST_DELAY      100     /**< Time to delay between each test */
...
class sipXtapiTestSuite : public CppUnit::TestFixture
{
   ...
   /* fails, setConnectionIdleTimeout is not supported by our CpMediaInterfaceFactoryImpl */
   ...
   CPPUNIT_TEST(testConfigCrlfKeepAliveFailure);
   CPPUNIT_TEST(testConfigStunKeepAlive);
   // TODO: add keepalive event validation
...
#if TEST_PROBLEMATIC_CASES /* [ */
   CPPUNIT_TEST(testCallRapidCallAndHangup);
...
public:
   ...
   void testConfigCrlfKeepAliveFailure();
