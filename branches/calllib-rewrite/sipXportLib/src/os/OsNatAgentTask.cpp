...
UtlBoolean OsNatAgentTask::handleCrLfKeepAlive(NAT_AGENT_CONTEXT* pContext) 
{
    ...
    {
        pContext->pKeepaliveListener->OnKeepaliveFeedback(populateKeepaliveEvent(pContext));
        bRC = true ;
    }
    else
    {
...
/*            {
               ...
               // we release timer later to prevent deadlock, as OsTimerTask
               // could be firing a timer sending a message to SipClient which has
               // full message queue. At that point, SipClient is trying to get
               // lock on mContextMap which we already have here in a Call...
               pBinding->pTimer->stop(FALSE);
               mTimersToRelease.append(new UtlVoidPtr(pBinding->pTimer));
            }*/
