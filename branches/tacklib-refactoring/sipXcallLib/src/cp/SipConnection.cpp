...
// Constructor
SipConnection::SipConnection(const UtlString& localAddress,
                             ...
                             : Connection(callMgr, call, mediaInterface, pCallEventListener,
                             pInfoStatusEventListener, pSecurityEventListener, pMediaEventListener,
                             offeringDelayMilliSeconds,
                             availableBehavior, forwardUnconditionalUrl,
                             busyBehavior, forwardOnBusyUrl, -1)
{
...
SIPX_CONTACT_TYPE SipConnection::selectCompatibleContactType(const SipMessage& request)
{
   ...
   else
   {
      ...
      m_mediaIPAddress = NULL; // will be autodetected by media stack if not set here
      getContactAdapterName(adapterName, m_bindIPAddress);
      ...
      {
         m_mediaIPAddress = stun_contact.cIpAddress;
      ...
      {
         m_mediaIPAddress = local_contact.cIpAddress;
                               ...
                               const RTP_TRANSPORT rtpTransportOptions)
{
   ...
   if(getState() == CONNECTION_IDLE && mpMediaInterface != NULL)
   {
      ...
      if (pAddress != NULL)
      {
         m_mediaIPAddress = pAddress->cIpAddress;
      }
      else
      {
         // we must select some mediaIPAddress from bind IP address
         if (m_bindIPAddress.compareTo("0.0.0.0") == 0)
         {
            m_mediaIPAddress = NULL; // use empty, it will be autodetected, we can't send 0.0.0.0 in SDP!
         }
         else
         {
            m_mediaIPAddress = m_bindIPAddress; // specific bind IP address was specified, use it
         }
      }
         ...
         rtpTransportOptions) != OS_SUCCESS)
      {
         ...
         if (!bAudioAvailable)
         {
            fireSipXMediaEvent(CP_MEDIA_DEVICE_FAILURE, CP_MEDIA_CAUSE_DEVICE_UNAVAILABLE, CP_MEDIA_TYPE_AUDIO);
         }
...
UtlBoolean SipConnection::renegotiateCodecs()
{
   if (mCSeqMgr.isInTransaction(CSEQ_ID_INVITE))
   {
      ...
      mpRenegotiateCodecsTimer->oneshotAfter(timerTime);
   }
   else
   {
      silentRemoteHold();
...
UtlBoolean SipConnection::extendSessionReinvite()
{
   ...
   if(inviteMsg && getState() == CONNECTION_ESTABLISHED)
   {
      ...
      else
      {
         ...
         mpReinviteTimer1->oneshotAfter(timerTime);
      }
   }
...
void SipConnection::processInviteRequest(const SipMessage* request)
{
   ...
   setBindIPAddress(request->getLocalIp().data());
   ...
   if(refIndex >= 0)
   {
         ...
         contentType.index(CONTENT_TYPE_MESSAGE_SIPFRAG, 0, UtlString::ignoreCase) == 0) )
      {
         ...
         if(responseCode == SIP_OK_CODE)
         {
            ...
            fireSipXCallEvent(CALLSTATE_TRANSFER_EVENT, CALLSTATE_CAUSE_TRANSFER_SUCCESS, NULL, responseCode, mResponseText);
         }
         else if(responseCode == SIP_DECLINE_CODE)
         {
            ...
            fireSipXCallEvent(CALLSTATE_TRANSFER_EVENT, CALLSTATE_CAUSE_TRANSFER_FAILURE, NULL, responseCode, mResponseText);
         }
                                          ...
                                          UtlBoolean callInFocus)
{
   ...
   if(!inviteMsg)
   {
      ...
      setState(CONNECTION_FAILED, CONNECTION_REMOTE);
      fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_NORMAL, NULL, responseCode, mResponseText);
   }
      ...
      strcmp(sequenceMethod.data(), SIP_CANCEL_METHOD) == 0)
   {
         ...
         sequenceMethod.compareTo(mLastRequestMethod) == 0)
      {
            ...
            previousState != CONNECTION_FAILED)
         {
            ...
            mpCancelTimer->oneshotAfter(timerTime);
         }
...
void SipConnection::processInviteResponseRinging(const SipMessage* response)
{
   ...
   */
   ...
   {
      fireSipXCallEvent(CALLSTATE_REMOTE_ALERTING, CALLSTATE_CAUSE_NORMAL, NULL, responseCode, mResponseText);
   }
}
...
void SipConnection::processInviteResponseBusy(const SipMessage* response)
{
   ...
   */
   setState(CONNECTION_FAILED, CONNECTION_REMOTE, CONNECTION_CAUSE_BUSY);
   fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_BUSY, NULL, responseCode, responseText);
...
void SipConnection::processInviteResponseRequestPending(const SipMessage* response)
{
   ...
   */
   ...
   mpReinviteTimer2->oneshotAfter(timerTime);
}
...
void SipConnection::processInviteResponseFailed(const SipMessage* response)
{
   ...
   if (reinviteState == ACCEPT_INVITE)
   {
      ...
      */
      ...
      else if (responseCode == SIP_NOT_FOUND_CODE)
      {
         ...
         setState(CONNECTION_DISCONNECTED, CONNECTION_REMOTE, cause);
         fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_BAD_ADDRESS, NULL, responseCode, responseText);
      }
      else if (responseCode == SIP_REQUEST_UNDECIPHERABLE_CODE)
      {
         cause = CONNECTION_CAUSE_NOT_ALLOWED;
         setState(CONNECTION_DISCONNECTED, CONNECTION_REMOTE, cause);
         fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_REMOTE_SMIME_UNSUPPORTED, NULL, responseCode, responseText);
      }
      else if (responseCode == SIP_REQUEST_NOT_ACCEPTABLE_HERE_CODE)
      {
         ...
         fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_SMIME_FAILURE, NULL, responseCode, responseText);
      }
      else if (responseCode == SIP_REQUEST_TIMEOUT_CODE)
      {
         setState(CONNECTION_FAILED, CONNECTION_REMOTE, cause);
         fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_NO_RESPONSE, NULL, responseCode, responseText);
      }
      ...
      {
         setState(CONNECTION_FAILED, CONNECTION_REMOTE, cause);
         fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_REQUEST_NOT_ACCEPTED, NULL, responseCode, responseText);
      }
      else
      {
         setState(CONNECTION_FAILED, CONNECTION_REMOTE, cause);
         fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_UNKNOWN, NULL, responseCode, responseText);
      }
      ...
      responseCode != SIP_REQUEST_NOT_ACCEPTABLE_HERE_CODE)
   {
      ...
      */
      ...
      fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_RESOURCE_LIMIT, NULL, responseCode, responseText);
   }
   else if (reinviteState == REINVITING &&
      responseCode == SIP_REQUEST_NOT_ACCEPTABLE_HERE_CODE)
   {
      ...
      */
      ...
      fireSipXCallEvent(CALLSTATE_CONNECTED, CALLSTATE_CAUSE_REQUEST_NOT_ACCEPTED, NULL, responseCode, responseText);
   }
}
   ...
   */
   ...
   else
   {
      ...
      if(mSessionReinviteTimer > 0)
      {
         ...
         mpReinviteTimer3->oneshotAfter(timerTime);
      }
      ...
      */
      ...
      if (numMatchingCodecs > 0 && mpMediaInterface != NULL)
      {
            ...
            mHoldState == TERMCONNECTION_HOLDING)
         {
            ...
            if (mpCall->isInFocus())
            {
               fireSipXCallEvent(CALLSTATE_REMOTE_HELD, CALLSTATE_CAUSE_NORMAL, NULL, responseCode, responseText);
            }
            else
            {
               fireSipXCallEvent(CALLSTATE_HELD, CALLSTATE_CAUSE_NORMAL, NULL, responseCode, responseText);
            }
         ...
         else
         {
            ...
            if (mpCall->isInFocus())
            {
               fireSipXCallEvent(CALLSTATE_CONNECTED, CALLSTATE_CAUSE_NORMAL, NULL, responseCode, responseText);
            }
            else
            {
   ...
   if (getState() != CONNECTION_FAILED)
   {
      ...
      if(!contactUri.isNull() && inviteMsg)
      {
         ...
         if(send(sipRequest))
         {
            ...
            fireSipXCallEvent(CALLSTATE_REMOTE_OFFERING, CALLSTATE_CAUSE_NORMAL, NULL, responseCode, responseText);
         }
         else
         {
            ...
            fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_NETWORK, NULL, responseCode, responseText);
         }
      }
      else
      {
         ...
         setState(CONNECTION_FAILED, CONNECTION_REMOTE);
         fireSipXCallEvent(CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_BAD_REDIRECT, NULL, responseCode, responseText);
      }
   }
}
...
void SipConnection::processReferResponse(const SipMessage* response)
{
   ...
   if (mCSeqMgr.getCSeqNumber(CSEQ_ID_REFER) == sequenceNum)
   {
      ...
      if(responseCode == SIP_OK_CODE)
      {
         ...
         fireSipXCallEvent(CALLSTATE_TRANSFER_EVENT, CALLSTATE_CAUSE_TRANSFER_ACCEPTED, NULL, responseCode, responseText);
      }
      ...
      else if(responseCode == SIP_DECLINE_CODE)
      {
         ...
         fireSipXCallEvent(CALLSTATE_TRANSFER_EVENT, CALLSTATE_CAUSE_TRANSFER_FAILURE, NULL, responseCode, responseText);
      }
      else if(responseCode == SIP_BAD_METHOD_CODE ||
         responseCode == SIP_UNIMPLEMENTED_METHOD_CODE)
      {
   ...
   if (mCSeqMgr.getCSeqNumber(CSEQ_ID_INVITE) == sequenceNum)
   {
      ...
      if(responseCode == SIP_TRYING_CODE)
      {
         ...
         CpMultiStringMessage* expiredBye =
            ...
            callId.data(), toField.data(), fromField.data());
         ...
         mpForceDropConnectionTimer1->oneshotAfter(timerTime);
      }
...
void SipConnection::processCancelResponse(const SipMessage* response)
{
   ...
   if (mCSeqMgr.getCSeqNumber(CSEQ_ID_INVITE) == sequenceNum)
   {
      ...
      if(responseCode == SIP_TRYING_CODE)
      {
         ...
         // Convert from mSeconds to uSeconds
         OsTime timerTime(sipUserAgent->getSipStateTransactionTimeout() / 1000, 0);
         mpForceDropConnectionTimer2->oneshotAfter(timerTime);
      }
      else if(responseCode >= SIP_2XX_CLASS_CODE)
      {
         // Stop sending & receiving RTP
                                                ...
                                                int              nMatchedCodces)
{
   ...
   {
      fireSipXMediaEvent(CP_MEDIA_REMOTE_STOP, CP_MEDIA_CAUSE_INCOMPATIBLE, CP_MEDIA_TYPE_AUDIO);
   }
   ...
   if (bIncludedVideoCodec && !bFoundVideoCodecs)
   {
      ...
      {
         fireSipXMediaEvent(CP_MEDIA_REMOTE_STOP, CP_MEDIA_CAUSE_INCOMPATIBLE, CP_MEDIA_TYPE_VIDEO);
      }
   }
}
...
void SipConnection::fireAudioStartEvents(CP_MEDIA_CAUSE cause)
{
   ...
   if (mpMediaInterface)
   {
         ...
         tapiCodec.m_bIsEncrypted) == OS_SUCCESS)
      {
         ...
         {
            fireSipXMediaEvent(CP_MEDIA_LOCAL_START, cause, CP_MEDIA_TYPE_AUDIO, (intptr_t)&tapiCodec);
         }
         if (mpMediaInterface->isSendingRtpVideo(mConnectionId))
         {
            fireSipXMediaEvent(CP_MEDIA_LOCAL_START, cause, CP_MEDIA_TYPE_VIDEO, (intptr_t)&tapiCodec);
         }
         if (mpMediaInterface->isReceivingRtpAudio(mConnectionId))
         {
            fireSipXMediaEvent(CP_MEDIA_REMOTE_START, cause, CP_MEDIA_TYPE_AUDIO, (intptr_t)&tapiCodec);
         }
         if (mpMediaInterface->isReceivingRtpVideo(mConnectionId))
         {
            fireSipXMediaEvent(CP_MEDIA_REMOTE_START, cause, CP_MEDIA_TYPE_VIDEO, (intptr_t)&tapiCodec);
         }
      }
   }
}
   ...
   if (mpMediaInterface)
   {
      if (mpMediaInterface->isAudioInitialized(mConnectionId))
      {
         ...
         */
         if (!mpMediaInterface->isSendingRtpAudio(mConnectionId))
         {
            fireSipXMediaEvent(CP_MEDIA_LOCAL_STOP, cause, CP_MEDIA_TYPE_AUDIO);
         }
         if (!mpMediaInterface->isReceivingRtpAudio(mConnectionId))
         {
            fireSipXMediaEvent(CP_MEDIA_REMOTE_STOP, cause, CP_MEDIA_TYPE_AUDIO);
         }
      }
      ...
      if (mpMediaInterface->isVideoInitialized(mConnectionId))
      {
         ...
         */
         if (!mpMediaInterface->isSendingRtpVideo(mConnectionId))
         {
            fireSipXMediaEvent(CP_MEDIA_LOCAL_STOP, cause, CP_MEDIA_TYPE_VIDEO);
         }
         if (!mpMediaInterface->isReceivingRtpVideo(mConnectionId))
         {
            fireSipXMediaEvent(CP_MEDIA_REMOTE_STOP, cause, CP_MEDIA_TYPE_VIDEO);
         }
      }
   }
}
