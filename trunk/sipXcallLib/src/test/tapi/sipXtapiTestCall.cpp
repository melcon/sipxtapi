...
void sipXtapiTestSuite::testCallMakeAPI()
{
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      ...
      EventValidator validatorCalling("testCallMakeAPI.calling");
      validatorCalling.ignoreEventCategory(EVENT_CATEGORY_LINESTATE);

      rc = sipxEventListenerAdd(g_hInst1, UniversalEventValidatorCallback, &validatorCalling);
      CPPUNIT_ASSERT(rc == SIPX_RESULT_SUCCESS);

      rc = sipxLineAdd(g_hInst1, "sip:bandreasen@pingtel.com", &hLine);
      CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
      CPPUNIT_ASSERT(hLine);

      rc = sipxCallCreate(g_hInst1, hLine, &hCall);
      CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
      ...
      SIPX_CALL hOldCall = hCall;
      rc = sipxCallDestroy(&hCall);
      CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
      CPPUNIT_ASSERT(hCall == SIPX_CALL_NULL);

      bRC = validatorCalling.waitForCallEvent(hLine, hOldCall, CALLSTATE_DESTROYED, CALLSTATE_CAUSE_NORMAL, true);
      CPPUNIT_ASSERT(bRC);

      rc = sipxLineRemove(hLine);
      CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);

      rc = sipxEventListenerRemove(g_hInst1, UniversalEventValidatorCallback, &validatorCalling);
      CPPUNIT_ASSERT(rc == SIPX_RESULT_SUCCESS);
   }
...
void sipXtapiTestSuite::testCallGetID()
{
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      ...
      // now try it with connected call
      ...
      // drop both calls
      destroyCall(hCallingLine, hCallingCall);
      destroyCall(hCalledLine, hCalledCall);
   }
   OsTask::delay(TEST_DELAY);
...
{
   CPPUNIT_ASSERT_EQUAL(sipxLineAdd(g_hInst1, "sip:bandreasen@pingtel.com", phLine), SIPX_RESULT_SUCCESS);
...
{
   CPPUNIT_ASSERT_EQUAL(sipxCallCreate(g_hInst1, hLine, phCall), SIPX_RESULT_SUCCESS);
...
void sipXtapiTestSuite::testCallBasic()
{
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      ...
      doCallBasic(g_hInst1, "sip:calling@127.0.0.1:8000", "sip:calling@127.0.0.1:8000",
...
// A calls B, B answers, A hangs up
void sipXtapiTestSuite::testCallPlayAudioFileFast()
{
   bool bRC;
   ...
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      printf("\ntestCallPlayAudioFileFast (%2d of %2d)", iStressFactor+1, STRESS_FACTOR);
      SIPX_CALL hCall;
      SIPX_LINE hLine;
      SIPX_LINE hReceivingLine;
...
// A calls B, B answers, A hangs up
void sipXtapiTestSuite::testCallPlayAudioFileSlow()
{
   bool bRC;
   EventValidator validatorCalling("testCallPlayAudioFile.calling");
   EventValidator validatorCalled("testCallPlayAudioFile.called");
   ...
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      printf("\ntestCallPlayAudioFileSlow (%2d of %2d)", iStressFactor+1, STRESS_FACTOR);
      SIPX_CALL hCall;
      SIPX_LINE hLine;
      SIPX_LINE hReceivingLine;
...
// A calls B, B answers, A hangs up
void sipXtapiTestSuite::testCallPlaybackFastPauseResume()
{
   bool bRC;
   EventValidator validatorCalling("testCallPlayAudioFile.calling");
   EventValidator validatorCalled("testCallPlayAudioFile.called");
   ...
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      printf("\ntestCallPlaybackFastPauseResume (%2d of %2d)", iStressFactor+1, STRESS_FACTOR);
      SIPX_CALL hCall;
      SIPX_LINE hLine;
      SIPX_LINE hReceivingLine;
      ...
      for (int i = 0; i < 100; i++)
      {
         ...
         sipxCallAudioPlayFileStop(hCall);
         bRC = validatorCalled.waitForMediaEvent(MEDIA_PLAYBACK_PAUSED, MEDIA_CAUSE_NORMAL, MEDIA_TYPE_AUDIO, true);
         sipxCallAudioPlaybackPause(g_hAutoAnswerCallbackCall);
         bRC = validatorCalled.waitForMediaEvent(MEDIA_PLAYBACK_PAUSED, MEDIA_CAUSE_NORMAL, MEDIA_TYPE_AUDIO, true);
         sipxCallAudioPlaybackResume(g_hAutoAnswerCallbackCall);
         sipxCallAudioPlayFileStop(hCall);
         bRC = validatorCalled.waitForMediaEvent(MEDIA_PLAYBACK_RESUMED, MEDIA_CAUSE_NORMAL, MEDIA_TYPE_AUDIO, true);
         sipxCallAudioPlaybackResume(g_hAutoAnswerCallbackCall);
         bRC = validatorCalled.waitForMediaEvent(MEDIA_PLAYBACK_RESUMED, MEDIA_CAUSE_NORMAL, MEDIA_TYPE_AUDIO, true);
         sipxCallAudioPlayFileStop(g_hAutoAnswerCallbackCall);
         sipxCallAudioPlayFileStop(hCall);
...
// A calls B, B answers, A hangs up
void sipXtapiTestSuite::testCallPlaybackSlowPauseResume()
{
   bool bRC;
   EventValidator validatorCalling("testCallPlayAudioFile.calling");
   EventValidator validatorCalled("testCallPlayAudioFile.called");
   ...
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      printf("\ntestCallPlaybackSlowPauseResume (%2d of %2d)", iStressFactor+1, STRESS_FACTOR);
      SIPX_CALL hCall;
      SIPX_LINE hLine;
      SIPX_LINE hReceivingLine;
...
void sipXtapiTestSuite::testCallHoldExceedingIdleTimeout()
{
   ...
   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      ...
      // Set idle timeout
      CPPUNIT_ASSERT(sipxConfigSetConnectionIdleTimeout(g_hInst1, 5) == SIPX_RESULT_SUCCESS);
...
void sipXtapiTestSuite::testManualCallDialtone()
{
   ...
   // Setup
   sipxLineAdd(g_hInst1, "sip:foo@10.1.1.120", &hLine);
   sipxCallCreate(g_hInst1, hLine, &hCall);
   ...
   EventValidator* pCalledPartyValidator)
{
   ...
   */
   ...
   bRC = pCallingPartyValidator->waitForMediaEvent(MEDIA_LOCAL_STOP, MEDIA_CAUSE_HOLD, MEDIA_TYPE_AUDIO, false);
   CPPUNIT_ASSERT(bRC);
   bRC = pCallingPartyValidator->waitForMediaEvent(MEDIA_REMOTE_STOP, MEDIA_CAUSE_HOLD, MEDIA_TYPE_AUDIO, false);
   CPPUNIT_ASSERT(bRC);
   bRC = pCallingPartyValidator->waitForCallEvent(hCallingPartyLine, hCallingParty, CALLSTATE_HELD, CALLSTATE_CAUSE_NORMAL, false);
   CPPUNIT_ASSERT(bRC);

   bRC = pCalledPartyValidator->waitForMediaEvent(MEDIA_LOCAL_STOP, MEDIA_CAUSE_HOLD, MEDIA_TYPE_AUDIO, false);
   CPPUNIT_ASSERT(bRC);
   bRC = pCalledPartyValidator->waitForMediaEvent(MEDIA_REMOTE_STOP, MEDIA_CAUSE_HOLD, MEDIA_TYPE_AUDIO, false);
   CPPUNIT_ASSERT(bRC);
   bRC = pCalledPartyValidator->waitForCallEvent(hCalledPartyLine, hCalledParty, CALLSTATE_REMOTE_HELD, CALLSTATE_CAUSE_NORMAL, false);
   CPPUNIT_ASSERT(bRC);
   ...
   */
   ...
   bRC = pCalledPartyValidator->waitForCallEvent(hCalledPartyLine, hCalledParty, CALLSTATE_HELD, CALLSTATE_CAUSE_NORMAL, true);
   CPPUNIT_ASSERT(bRC);
   ...
   */
   ...
   bRC = pCallingPartyValidator->waitForCallEvent(hCallingPartyLine, hCallingParty, CALLSTATE_REMOTE_HELD, CALLSTATE_CAUSE_NORMAL, true);
   ...
   EventValidator* pCalledPartyValidator)
{
   ...
   */
   ...
   bRC = pCallingPartyValidator->waitForCallEvent(hCallingPartyLine, hCallingParty, CALLSTATE_HELD, CALLSTATE_CAUSE_NORMAL, true);
   CPPUNIT_ASSERT(bRC);
