...
void sipXtapiTestSuite::testGainAPI() 
{
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        // Set to min
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetGain(g_hInst1, GAIN_MIN), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetGain(g_hInst1, &iGainLevel), SIPX_RESULT_SUCCESS) ;    
        CPPUNIT_ASSERT_EQUAL(iGainLevel, GAIN_MIN) ;

        // Set to default
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetGain(g_hInst1, GAIN_DEFAULT), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetGain(g_hInst1, &iGainLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iGainLevel, GAIN_DEFAULT) ;
        
        // set to max
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetGain(g_hInst1, GAIN_MAX), SIPX_RESULT_SUCCESS) ;    
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetGain(g_hInst1, &iGainLevel), SIPX_RESULT_SUCCESS) ;
...
void sipXtapiTestSuite::testMuteAPI()
{
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            // Test Mute API
            CPPUNIT_ASSERT_EQUAL(sipxAudioMute(g_hInst1, true), SIPX_RESULT_SUCCESS) ;
            CPPUNIT_ASSERT_EQUAL(sipxAudioIsMuted(g_hInst1, &bMuted), SIPX_RESULT_SUCCESS) ;
            CPPUNIT_ASSERT_EQUAL(bMuted, TRUE) ;
            CPPUNIT_ASSERT_EQUAL(sipxAudioGetGain(g_hInst1, &iGainLevel), SIPX_RESULT_SUCCESS) ;
            CPPUNIT_ASSERT_EQUAL(iGainLevel, 0) ;// when muted gain is 0

            // Test Unmute API
            CPPUNIT_ASSERT_EQUAL(sipxAudioMute(g_hInst1, false), SIPX_RESULT_SUCCESS) ;
            CPPUNIT_ASSERT_EQUAL(sipxAudioIsMuted(g_hInst1, &bMuted), SIPX_RESULT_SUCCESS) ;
...
void sipXtapiTestSuite::testVolumeAPI() 
{
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        // Test Enable Speaker
        CPPUNIT_ASSERT_EQUAL(sipxAudioEnableSpeaker(g_hInst1, SPEAKER), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetEnabledSpeaker(g_hInst1, &type), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(type, SPEAKER) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioEnableSpeaker(g_hInst1, RINGER), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetEnabledSpeaker(g_hInst1, &type), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(type, RINGER) ;

        // Set both RINGER and SPEAKER to know states (cloned below)
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetVolume(g_hInst1, SPEAKER, VOLUME_DEFAULT), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetVolume(g_hInst1, RINGER, VOLUME_DEFAULT), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, SPEAKER, &iLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iLevel, VOLUME_DEFAULT) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, RINGER, &iLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iLevel, VOLUME_DEFAULT) ;

        // Test SPEAKER making sure RINGER doesn't change
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetVolume(g_hInst1, SPEAKER, VOLUME_MIN), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, SPEAKER, &iLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iLevel, VOLUME_MIN) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, RINGER, &iLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iLevel, VOLUME_DEFAULT) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetVolume(g_hInst1, SPEAKER, VOLUME_MAX), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, SPEAKER, &iLevel), SIPX_RESULT_SUCCESS) ;
        ...
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetVolume(g_hInst1, RINGER, VOLUME_MIN), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, RINGER, &iLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iLevel, VOLUME_MIN) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, SPEAKER, &iLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iLevel, VOLUME_DEFAULT) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioSetVolume(g_hInst1, RINGER, VOLUME_MAX), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(sipxAudioGetVolume(g_hInst1, RINGER, &iLevel), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL(iLevel, VOLUME_MAX) ;
    }
...
void sipXtapiTestSuite::testAudioSettings()
{
    ...
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        SIPX_AEC_MODE mode ;
        CPPUNIT_ASSERT_EQUAL( sipxAudioSetAECMode(g_hInst1, SIPX_AEC_DISABLED), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetAECMode(g_hInst1, &mode), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL( mode, SIPX_AEC_DISABLED) ;

        CPPUNIT_ASSERT_EQUAL( sipxAudioSetAECMode(g_hInst1, SIPX_AEC_SUPPRESS), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetAECMode(g_hInst1, &mode), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL( mode, SIPX_AEC_SUPPRESS) ;

        CPPUNIT_ASSERT_EQUAL( sipxAudioSetAECMode(g_hInst1, SIPX_AEC_CANCEL), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetAECMode(g_hInst1, &mode), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL( mode, SIPX_AEC_CANCEL) ;

        CPPUNIT_ASSERT_EQUAL( sipxAudioSetAECMode(g_hInst1, SIPX_AEC_CANCEL_AUTO), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetAECMode(g_hInst1, &mode), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL( mode, SIPX_AEC_CANCEL_AUTO) ;

        // Test Noise Reduction
        SIPX_NOISE_REDUCTION_MODE nrMode ;
        CPPUNIT_ASSERT_EQUAL( sipxAudioSetNoiseReductionMode(g_hInst1, SIPX_NOISE_REDUCTION_DISABLED), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetNoiseReductionMode(g_hInst1, &nrMode), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL( nrMode, SIPX_NOISE_REDUCTION_DISABLED) ;

        CPPUNIT_ASSERT_EQUAL( sipxAudioSetNoiseReductionMode(g_hInst1, SIPX_NOISE_REDUCTION_LOW), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetNoiseReductionMode(g_hInst1, &nrMode), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL( nrMode, SIPX_NOISE_REDUCTION_LOW) ;

        CPPUNIT_ASSERT_EQUAL( sipxAudioSetNoiseReductionMode(g_hInst1, SIPX_NOISE_REDUCTION_MEDIUM), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetNoiseReductionMode(g_hInst1, &nrMode), SIPX_RESULT_SUCCESS) ;
        CPPUNIT_ASSERT_EQUAL( nrMode, SIPX_NOISE_REDUCTION_MEDIUM) ;

        CPPUNIT_ASSERT_EQUAL( sipxAudioSetNoiseReductionMode(g_hInst1, SIPX_NOISE_REDUCTION_HIGH), SIPX_RESULT_SUCCESS);
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetNoiseReductionMode(g_hInst1, &nrMode), SIPX_RESULT_SUCCESS) ;
        ...
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetNumInputDevices(g_hInst1, &numOfDevices), SIPX_RESULT_SUCCESS);
        for(i = 0; i < numOfDevices; i++) 
        {
            CPPUNIT_ASSERT_EQUAL( sipxAudioGetInputDevice(g_hInst1, i, &szDevice), SIPX_RESULT_SUCCESS);
        }
        ...
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetNumOutputDevices(g_hInst1, &numOfDevices), SIPX_RESULT_SUCCESS);
        for(i = 0; i < numOfDevices; i++) 
        {
            CPPUNIT_ASSERT_EQUAL( sipxAudioGetOutputDevice(g_hInst1, i, &szDevice), SIPX_RESULT_SUCCESS);
        }
        //checks if there are no more devices
        CPPUNIT_ASSERT_EQUAL( sipxAudioGetOutputDevice(g_hInst1, i, &szDevice), SIPX_RESULT_SUCCESS);
        //checker is NULL
...
void sipXtapiTestSuite::testSetCallback()
{
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        sipxLogEntryAdd(LOG_LEVEL_DEBUG, "**Testing callbacks**");
        OsTask::delay(200);
...
void sipXtapiTestSuite::testConfigExternalTransport() 
{
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        size_t nContacts ;
        size_t origContacts;
        sipxConfigGetLocalContacts(g_hInst1, addresses, 32, &origContacts) ;
        const char* szTransport = "flibble";
        const char* szLocalIp = "127.0.0.1";

        // verify that there are no custom transports yet
        memset(addresses, 0, sizeof(SIPX_CONTACT_ADDRESS)*32);
        sipxConfigGetLocalContacts(g_hInst1, addresses, 32, &nContacts);
        ...
        // when transport is added, a non zero handle is generated
        CPPUNIT_ASSERT((int)hTransport > 0);
        
        memset(addresses, 0, sizeof(SIPX_CONTACT_ADDRESS)*32);
        sipxConfigGetLocalContacts(g_hInst1, addresses, 32, &nContacts) ;
        CPPUNIT_ASSERT(nContacts == origContacts + 1 );

        // check that a correct custom transport was added
        bool bFound = false;
        for (size_t i = 0; i < nContacts; i++)
        {
           // find new custom contact
           if(addresses[i].eTransportType == TRANSPORT_CUSTOM)
           {
              CPPUNIT_ASSERT(strcmp(addresses[i].cCustomTransportName, szTransport) == 0);
              CPPUNIT_ASSERT(strcmp(addresses[i].cIpAddress, szLocalIp) == 0);
...
void sipXtapiTestSuite::testReinitializeCall() 
{
    ...
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        SIPX_INST hOldInst1 = hInst1;
        SIPX_INST hOldInst2 = hInst2;
        ...
        CPPUNIT_ASSERT(bRC);
        bRC = validatorCalling.waitForCallEvent(hCallingLine, hCallingCall, CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_NORMAL, true);
        CPPUNIT_ASSERT(bRC);
        bRC = validatorCalling.waitForCallEvent(hCallingLine, hCallingCall, CALLSTATE_DESTROYED, CALLSTATE_CAUSE_NORMAL, true);
        CPPUNIT_ASSERT(bRC);
        ...
        CPPUNIT_ASSERT(bRC);
        bRC = validatorCalled.waitForCallEvent(hCalledLine, hCalledCall, CALLSTATE_DISCONNECTED, CALLSTATE_CAUSE_NORMAL, true);
        CPPUNIT_ASSERT(bRC);
        bRC = validatorCalled.waitForCallEvent(hCalledLine, hCalledCall, CALLSTATE_DESTROYED, CALLSTATE_CAUSE_NORMAL, true);
        CPPUNIT_ASSERT(bRC);

/*        CPPUNIT_ASSERT_EQUAL(SipXEventDispatcher::getListenerCount(hOldInst1), 0);
        CPPUNIT_ASSERT_EQUAL(SipXEventDispatcher::getListenerCount(hOldInst2), 0);*/
...
void sipXtapiTestSuite::testConfigCodecPreferences()
{
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        sipxEventListenerAdd(g_hInst1, codec_CallBack_Place, NULL) ;
        ...
        CPPUNIT_ASSERT_EQUAL(sipxEventListenerRemove(g_hInst1, codec_CallBack_Place, NULL), SIPX_RESULT_SUCCESS) ;
...
void sipXtapiTestSuite::testConfigEnableStunNoResponse() 
{
    ...
    EventValidator stunServerValidator("validator") ;
    EventValidator stunClientValidator("validator") ;
    ...
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        setupStunServer() ;
        setStunServerValidator(&stunServerValidator) ;
        setStunServerMode(TEST_NO_RESPONSE) ;        
        
        rc = sipxInitialize(&hInst, 11234, 11234, 8031, 8050) ;
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
        rc = sipxEventListenerAdd(hInst, UniversalEventValidatorCallback, &stunClientValidator) ;
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
        ...
        stunServerValidator.addMarker("Waiting for STUN failure") ;
        rc = sipxConfigEnableStun(hInst, "127.0.0.1", DEFAULT_STUN_PORT, 10) ;
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS) ;

        stunServerValidator.setMaxLookhead(20) ;
        ...
        for (size_t i=0; i<nContacts; i++)
        {
            ...
            {
                // SUCCESSes may come in in a different rate
                bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::handleStunMessage", false) ;
                CPPUNIT_ASSERT(bRC) ;
                bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::Dropping Response", false) ;
                CPPUNIT_ASSERT(bRC) ;
                bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::handleStunMessage", false) ;
                CPPUNIT_ASSERT(bRC) ;
                bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::Dropping Response", false) ;
                ...
                bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::Dropping Response", false) ;
                CPPUNIT_ASSERT(bRC) ;
                bRC = stunClientValidator.waitForConfigEvent(CONFIG_STUN_FAILURE, false) ;
                CPPUNIT_ASSERT(bRC) ;
            }
        }
        ...
        rc = sipxEventListenerRemove(hInst, UniversalEventValidatorCallback, &stunClientValidator) ;
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
...
//    OsSocket::getHostIpByName("sbc.pingtel.com", &serverAddress) ;
    // keepaliveadd requires IP address
    OsSocket::getHostIpByName("155.212.198.121", &serverAddress) ;

    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        printf("\ntestConfigCrlfKeepAlive (%2d of %2d)", iStressFactor+1, STRESS_FACTOR);        

        EventValidator crlfClientValidator("crlfClientValidator");
        crlfClientValidator.setMaxLookhead(50);

        rc = sipxInitialize(&hInst, 0, 0, 8031, 8050) ;
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);

        rc = sipxEventListenerAdd(hInst, UniversalEventValidatorCallback, &crlfClientValidator);
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);

        rc = sipxConfigKeepAliveAdd(hInst, 1, SIPX_KEEPALIVE_CRLF, serverAddress, 5060, 1) ;
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);

        bRC = crlfClientValidator.waitForKeepaliveEvent(KEEPALIVE_START, KEEPALIVE_CAUSE_NORMAL,
           SIPX_KEEPALIVE_CRLF, serverAddress, 5060, 1, true);
        CPPUNIT_ASSERT(bRC);

        OsTask::delay(3000) ;

        // there should be at least one successful feedback
        bRC = crlfClientValidator.waitForKeepaliveEvent(KEEPALIVE_FEEDBACK, KEEPALIVE_CAUSE_NORMAL,
           SIPX_KEEPALIVE_CRLF, serverAddress, 5060, 1, true);
        CPPUNIT_ASSERT(bRC);
        ...
        bRC = crlfClientValidator.waitForKeepaliveEvent(KEEPALIVE_STOP, KEEPALIVE_CAUSE_NORMAL,
           SIPX_KEEPALIVE_CRLF, serverAddress, 5060, 1, false);
        CPPUNIT_ASSERT(bRC);

        rc = sipxConfigKeepAliveRemove(hInst, 1, SIPX_KEEPALIVE_CRLF, serverAddress, 5060) ;
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_FAILURE);

        rc = sipxEventListenerRemove(hInst, UniversalEventValidatorCallback, &crlfClientValidator);
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
...
void sipXtapiTestSuite::testConfigCrlfKeepAliveFailure() 
{
   SIPX_INST hInst = NULL ;
   SIPX_RESULT rc ;
   bool bRC;
   ...
   OsSocket::getHostIpByName("this.host.doesnt.exist", &serverAddress) ;

   for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
   {
      printf("\ntestConfigCrlfKeepAliveFailure (%2d of %2d)", iStressFactor+1, STRESS_FACTOR);        

      EventValidator crlfClientValidator("crlfClientValidator");
      crlfClientValidator.setMaxLookhead(12);
      ...
      // there should be at least one failed feedback
      bRC = crlfClientValidator.waitForKeepaliveEvent(KEEPALIVE_FAILURE, KEEPALIVE_CAUSE_NORMAL,
         SIPX_KEEPALIVE_CRLF, serverAddress, 5060, 1, true);
      CPPUNIT_ASSERT(bRC);
...
void sipXtapiTestSuite::testConfigStunKeepAlive()
{
    ...
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        EventValidator stunServerValidator("stunServerValidator");
        EventValidator stunClientValidator("stunClientValidator");
        stunClientValidator.setMaxLookhead(12);
        ...
        bRC = stunClientValidator.waitForKeepaliveEvent(KEEPALIVE_START, KEEPALIVE_CAUSE_NORMAL,
           SIPX_KEEPALIVE_STUN, "127.0.0.1", 3478, 1, true);
        CPPUNIT_ASSERT(bRC);

        bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::handleStunMessage", true);
        CPPUNIT_ASSERT(bRC);
        bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::Sending Response", true);
        CPPUNIT_ASSERT(bRC);
        bRC = stunClientValidator.waitForKeepaliveEvent(KEEPALIVE_FEEDBACK, KEEPALIVE_CAUSE_NORMAL,
            SIPX_KEEPALIVE_STUN, "127.0.0.1", 3478, 1, true);
        CPPUNIT_ASSERT(bRC);
        bRC = stunServerValidator.waitForMessage(0, "TestStunServerTask::handleStunMessage", true);
        CPPUNIT_ASSERT(bRC);
        ...
        CPPUNIT_ASSERT_EQUAL(rc, SIPX_RESULT_SUCCESS);
        bRC = stunClientValidator.waitForKeepaliveEvent(KEEPALIVE_STOP, KEEPALIVE_CAUSE_NORMAL,
           SIPX_KEEPALIVE_STUN, "127.0.0.1", 3478, 1, false);
        CPPUNIT_ASSERT(bRC);
...
void sipXtapiTestSuite::testConfigStunKeepAliveOnce()
{
    ...
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        bRC = !stunServerValidator.validateNoWaitingEvent() ;
        CPPUNIT_ASSERT(bRC) ;
...
void sipXtapiTestSuite::testConfigEnableShortNames() 
{
    ...
    for (int iStressFactor = 0; iStressFactor<STRESS_FACTOR; iStressFactor++)
    {
        ...
        sipxConfigEnableSipShortNames(g_hInst1, true);
