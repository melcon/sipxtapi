...
const char *res_proto(int proto)
{
   switch (proto) {
        case 1: return("icmp");
        case 2: return("igmp");
...
int which_res_type(const char* str)
{
...
const char *res_class(int class)
{
...
#define PLURALIZE(x)    x, (x == 1) ? "" : "s"
   p = retbuf;
   if (days) {
      SNPRINTF(p, bufferSize, "%d day%s", PLURALIZE(days));
      while (*++p);
   }
   if (hours) {
      if (days)
         *p++ = ' ';
      SNPRINTF(p, bufferSize, "%d hour%s", PLURALIZE(hours));
      while (*++p);
   }
   if (mins) {
      if (days || hours)
         *p++ = ' ';
      SNPRINTF(p, bufferSize, "%d min%s", PLURALIZE(mins));
      while (*++p);
   }
   if (secs || ! (days || hours || mins)) {
      if (days || hours || mins)
         *p++ = ' ';
      SNPRINTF(p, bufferSize, "%d sec%s", PLURALIZE(secs));
   }
}
