...
{    
   m_events[m_numEvents++] = strdup(sipxConfigEventToString(pConfigInfo->event));
}

void EventRecorder::addCompareEvent(SIPX_CONFIG_INFO* pConfigInfo)
{
   m_compareEvents[m_numCompareEvents++] = strdup(sipxConfigEventToString(pConfigInfo->event));
}
