...
bool sipXmgr::handleSecurityEvent(void* pInfo, void* pUserData)
{
    ...
    bool bRet = true;
    char szMessage[768];
    const char* szEvent = sipxSecurityEventToString(pSecurityMsg->event);
    const char* szCause = sipxSecurityCauseToString(pSecurityMsg->cause);
...
bool sipXmgr::handleMediaEvent(void* pInfo, void* pUserData)
{
    ...
    sprintf(szMessage, "%s - %s\n",
       sipxMediaEventToString(pMediaInfo->event),
       sipxMediaCauseToString(pMediaInfo->cause));
...
bool sipXmgr::disconnect(SIPX_CALL hCall, bool bDisconnectAll)
{
    ...
    if (m_hConf != 0)
    {
        if (!bDisconnectAll)
        {
           SIPX_CALL hRemovedCall = hCall;
            sipxConferenceRemove(m_hConf, &hRemovedCall);
        ...
        else
        {
            ...
            {
               SIPX_CALL hRemovedCall = aCall[i];
                sipxConferenceRemove(m_hConf, &hRemovedCall);
            }
            sipxConferenceDestroy(m_hConf);
            m_hConf = 0;
...
bool sipXmgr::removeConfParty(const char* const szParty)
{
    ...
    if (pHandle)
    {
       SIPX_CALL hRemovedCall = pHandle->getValue();
       
       if (SIPX_RESULT_SUCCESS == sipxConferenceRemove(m_hConf, &hRemovedCall))
       {
         bRet = true;
       }
    }   
